...
| consumer-fileRecorder-* | dropEmptyPackets | int | 0 | If 1, memory pages are scanned and empty packets are discarded, i.e. packets with only an RDH, when RDH.memorySize==sizeof(RDH). Empty packets with stopBit set following a non-empty packet are kept. This setting does not change the content of in-memory data pages, other consumers would still get full data pages with empty packets. This setting is meant to reduce the amount of data recorded for continuous detectors in triggered mode.|
