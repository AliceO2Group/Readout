...
ReadoutEquipment::ReadoutEquipment(ConfigFile &cfg, std::string cfgEntryPoint) {
  ...
  // log config summary
  theLog.log(LogInfoDevel_(3002), "Equipment %s: from config [%s], max rate=%lf Hz, "
             "idleSleepTime=%d us, outputFifoSize=%d",
             name.c_str(), cfgEntryPoint.c_str(), readoutRate, cfgIdleSleepTime,
             cfgOutputFifoSize);
  theLog.log(LogInfoDevel_(3008), "Equipment %s: requesting memory pool %d pages x %d bytes from "
  ...
  if (disableOutput) {
    theLog.log(LogWarningDevel_(3002), "Equipment %s: output DISABLED ! Data will be readout and "
               "dropped immediately",
               name.c_str());
  }
...
void ReadoutEquipment::finalCounters(){
  if (cfgRdhCheckEnabled) {
    theLog.log(LogInfoDevel_(3003), "Equipment %s : %llu timeframes, "
