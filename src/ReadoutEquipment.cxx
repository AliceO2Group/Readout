...
ReadoutEquipment::ReadoutEquipment(ConfigFile &cfg, std::string cfgEntryPoint) {
  ...
  // log config summary
  theLog.log(LogInfoDevel_(3002), "Equipment %s: from config [%s], max rate=%lf Hz, idleSleepTime=%d us, outputFifoSize=%d",
             name.c_str(), cfgEntryPoint.c_str(), readoutRate, cfgIdleSleepTime,
             cfgOutputFifoSize);
  theLog.log(LogInfoDevel_(3008), "Equipment %s: requesting memory pool %d pages x %d bytes from bank '%s', block aligned @ 0x%X, 1st page offset @ 0x%X",
  ...
  if (disableOutput) {
    theLog.log(LogWarningDevel_(3002), "Equipment %s: output DISABLED ! Data will be readout and dropped immediately",
               name.c_str());
  }
  ...
  timeframePeriodOrbits = cfgTFperiod;
    ...
    if (usingSoftwareClock) {
      ...
      theLog.log(LogInfoDevel_(3002), "Timeframe IDs generated by software, %.2lf Hz",  
                 timeframeRate);
    } else {
  ...
  theLog.log(LogInfoDevel_(3008), "pageSpaceReserved = %d, aligning 1st page @ 0x%X",
...
void ReadoutEquipment::stop() {
  ...
  for (int i = 0; i < (int)EquipmentStatsIndexes::maxIndex; i++) {
    if (equipmentStats[i].getCount()) {
      theLog.log(LogInfoDevel_(3003), "%s.%s = %llu  (avg=%.2lf  min=%llu  max=%llu  count=%llu)",
      ...
      theLog.log(LogInfoDevel_(3003), "%s.%s = %llu", name.c_str(), EquipmentStatsNames[i],
                 (unsigned long long)equipmentStats[i].get());
    }
  }

  theLog.log(LogInfoDevel_(3003), "Average pages pushed per iteration: %.1f",
  ...
  for (;;) {
    ...
    for (int i = 0; i < maxBlocksToRead; i++) {
      ...
      if (ptr->debugFirstPages > 0) {
        ...
        theLog.log(LogDebugDevel_(3009),"Equipment %s (%d) page %lu link %d tf %lu size %d",
    ...
    if (ptr->cfgConsoleStatsUpdateTime > 0) {
      if (ptr->consoleStatsTimer.isTimeout()) {
        for (int i = 0; i < (int)EquipmentStatsIndexes::maxIndex; i++) {
          ...
          theLog.log(LogInfoDevel_(3003), "%s.%s : diff=%llu total=%llu", ptr->name.c_str(),
...
void ReadoutEquipment::finalCounters(){
  if (cfgRdhCheckEnabled) {
    theLog.log(LogInfoDevel_(3003), "Equipment %s : %llu timeframes, RDH checks %llu ok, %llu errors, %llu stream inconsistencies",
  ...
  if (tfId != currentTimeframe) {
    ...
    if (tfId != currentTimeframe + 1) {
      if (cfgRdhDumpWarningEnabled) {
        theLog.log(LogWarningSupport_(3004),
    ...
    for (size_t pageOffset = 0; pageOffset < blockSize;) {
	 ...
	 || ((blockHeader.timeframeOrbitFirst > blockHeader.timeframeOrbitLast) && 
        ...
        if (cfgRdhDumpErrorEnabled) {
          theLog.log(LogWarningSupport_(3004),
                     "RDH #%d @ 0x%X : TimeFrame ID change in page not allowed : orbit 0x%08X not in range [0x%08X,0x%08X]",
