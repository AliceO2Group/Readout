...
public:
  ConsumerFileRecorder(ConfigFile &cfg, std::string cfgEntryPoint)
    ...
    // | consumer-fileRecorder-* | dropEmptyPackets | int | 0 | If 1, memory pages are scanned and empty packets are discarded, i.e. packets with only an RDH, when RDH.memorySize==sizeof(RDH). This setting does not change the content of in-memory data pages, other consumers would still get full data pages with empty packets. This setting is meant to reduce the amount of data recorded for continuous detectors in triggered mode.|
    cfg.getOptionalValue(cfgEntryPoint + ".dropEmptyPackets",
                         dropEmptyPackets, 0);
    if (dropEmptyPackets) {
      theLog.log("Packets with RDH-only payload will not be recorded to file, option dropEmptyPackets is enabled");
    }
  ...
  ~ConsumerFileRecorder() {
    ...
    if (dropEmptyPackets) {
      theLog.log("Packets recorded=%lld discarded(empty)=%lld",packetsRecorded,emptyPacketsDropped);
    }
  }
	  ...
	  bool isFirstPacket = true;
	    ...
	    if (memorySize!=headerSize) {
	      ...
	      ptr = baseAddress + pageOffset;
              size = offsetNextPacket; // use offsetNextPacket instead of memorySize for file to be consistent
              status = fpUsed->write(ptr, size, isFirstPacket); // payload count as a page
	      isFirstPacket = false; // we have flagged the first packet in page already
	      packetsRecorded++;
	      if (status != FileHandle::Status::Success) {
...
private:
  ...
  int dropEmptyPackets = 0; // if set, packets with RDH.memorysize=sizeof(RDH) are discarded
