...
ReadoutEquipmentPlayer::ReadoutEquipmentPlayer(ConfigFile &cfg, std::string cfgEntryPoint) : ReadoutEquipment(cfg, cfgEntryPoint) {
  ...
  // log config summary
  theLog.log("Equipment %s: using data source file=%s preLoad=%d fillPage=%d", name.c_str(), filePath.c_str(),preLoad,fillPage);
  ...
  if (usablePageSize<fileSize) {
    throw(std::string("memoryPoolPageSize too small, need at least ")+std::to_string(fileSize+sizeof(DataBlock))+std::string(" bytes"));
  }
