...
ROCdevice::ROCdevice(std::string id) {
  ...
  theLog.log("ROC PCI %s @ NUMA node %d, serial number %s, firmware version %s, card id %s", infoPciAddress.c_str(), infoNumaNode, infoSerialNumber.c_str(), infoFirmwareVersion.c_str(), infoCardId.c_str());
...
int main(int argc, char**argv) {
  ...
  // synchronize with clock: start on next round(10s) to sync with possible other instances
