...
//    cfg.getOptionalValue<int>(cfgEntryPoint + ".remotePageSize", cfgRemotePageSize);
    ...
    if (!device_list) { throw __LINE__; }
    for (int i = 0; i < num_devices; ++ i) {
      theLog.log("RDMA device[%d]: name=%s", i, ibv_get_device_name(device_list[i]));
    }
    ...
    if (cm_channel==nullptr) {throw __LINE__;}   
    if (rdma_create_id(cm_channel, &cm_id, NULL, RDMA_PS_TCP)) {throw __LINE__;}
...
*/  
    ...
    if (getaddrinfo(cfgHost.c_str(), cfgPort.c_str(), &addr_hints, &addr_res)<0) {throw __LINE__;}
    ...
    if (addr_res!=nullptr) {freeaddrinfo(addr_res);}
    if (!addr_found) {throw __LINE__;}

    struct rdma_cm_event *event=nullptr;
    if (rdma_get_cm_event(cm_channel, &event)) {throw __LINE__;}
    //theLog.log("event:%s",rdma_event_str(event->event));
    if (event->event != RDMA_CM_EVENT_ADDR_RESOLVED) {throw __LINE__;}
    rdma_ack_cm_event(event);
    
    if (rdma_resolve_route(cm_id, RESOLVE_TIMEOUT_MS)) {throw __LINE__;}
    if (rdma_get_cm_event(cm_channel, &event)) {throw __LINE__;}
    if (event->event != RDMA_CM_EVENT_ROUTE_RESOLVED) {throw __LINE__;}
    rdma_ack_cm_event(event);
    ...
    int port_num = 1;
    if (ibv_query_port(cm_id->verbs, port_num, &port_attr)) {throw __LINE__;}
    ...
    if (pd==nullptr) {throw __LINE__;}

    // create completion event channel
    comp_chan = ibv_create_comp_channel(cm_id->verbs);
    if (comp_chan==nullptr) {throw __LINE__;}
    
    // set completion channel non-blocking
    int flags = fcntl(comp_chan->fd, F_GETFL);
    if (fcntl(comp_chan->fd, F_SETFL, flags | O_NONBLOCK)<0) {throw __LINE__;}
    ...
    if (rdma_create_qp(cm_id, pd, &qp_attr)) {throw __LINE__;}
    ...
    if (rdma_connect(cm_id, &conn_param)) {throw __LINE__;}
    if (rdma_get_cm_event(cm_channel,&event)) {throw __LINE__;}
    printf("event:%s\n",rdma_event_str(event->event));
    if (event->event != RDMA_CM_EVENT_ESTABLISHED) {throw __LINE__;}
    ...
    theLog.log("Remote buffer : %lu bytes total, %lu pages x %lu bytes",server_pdata.buf_pageSize*server_pdata.buf_numberOfPages,server_pdata.buf_numberOfPages,server_pdata.buf_pageSize);
...
//	nBytesSent+=b->getData()->header.dataSize;
     ...
     startCQHandle:
      ...
      struct ibv_cq *evt_cq=nullptr;
      void *cq_context=nullptr;
      if (ibv_get_cq_event(comp_chan,&evt_cq, &cq_context)) {goto endCQHandle;}
      //printf("got CQ event\n");
...
//      printf("%d available\n",nAvailable.load());
      ...
      //printf("Write remote @ page[%d]=%p\n",nPagesSent % server_pdata.buf_numberOfPages,(void *)(wr.wr.rdma.remote_addr));
/*      printf("Write remote @ %p\n",(void *)(wr.wr.rdma.remote_addr));
  ...
  */
      ...
      if (ibv_post_send(cm_id->qp, &wr, &bad_wr)) {throw __LINE__;}
